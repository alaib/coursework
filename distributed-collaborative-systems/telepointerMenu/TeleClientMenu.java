...
public class TeleClientMenu{
	...
	public class TeleClientModel{
		TeleClientMenu tc;
		public TeleClientModel(TeleClientMenu tClient){
			tc = tClient;
			try {
	...
	public class TeleClientView{
		...
		public JMenu menu;
		public JMenuItem display, hide;
		public Container contentPane;
		public JCheckBox toggleBtn;
		public JEditorPane editorPane;		
		public JFrame loginFrame;
		public TeleClientMenu tc;
		
		public TeleClientView(TeleClientMenu tClient){
			tc = tClient;
			initLoginScreen();
		}								
		...
		public void setupGlassPane(){
	        ...
	        menu = new JMenu("Menu");
	        display = new JMenuItem("Display TelePointer");
	        hide = new JMenuItem("Hide TelePointer");
	        menu.add(display);
	        menu.add(hide);
	        menuBar.add(menu);
			...
			gPane = new MyGlassPane(tc, frame, menu, display, hide, toggleBtn, editorPane, contentPane);
	...
	public class MyGlassPane extends JComponent implements ItemListener{
		...
		public MyGlassPane(TeleClientMenu tClient, JFrame frame, JMenu menuBar, JMenuItem display, JMenuItem hide, AbstractButton toggleBtn, JEditorPane editorPane, Container contentPane){
			tc = tClient;
			listener = new PaneListener(tc, frame, menuBar, display, hide, toggleBtn, editorPane, this, contentPane);
	...
	public class PaneListener extends MouseInputAdapter{
		...
		JMenu menu;
		JMenuItem display, hide;
		JFrame frame;
		Point prevPoint;
		TeleClientMenu tc;
		
		public PaneListener(TeleClientMenu tClient, JFrame f, JMenu m, JMenuItem d, JMenuItem h, Component tBtn, JEditorPane ePane, MyGlassPane gPane, Container cPane){
	    ...
	    private void redispatchMouseEvent(MouseEvent e, boolean repaint) {	    	
	        ...
	        if(menu.isPopupMenuVisible()){
	        	limit = menu.getHeight()+display.getHeight()+hide.getHeight();
	        }
	        if (containerPoint.y < limit) {
	        	//This mouse event is over the menu bar	        		        			        			        			        	
        		Point componentPoint = SwingUtilities.convertPoint(glassPane, glassPanePoint, frame);
        		//Menu Bar
        		if(componentPoint.y <= menu.getHeight() && componentPoint.x <= menu.getWidth()){
	        		MouseEvent me = new MouseEvent(menu, e.getID(), e.getWhen(), e.getModifiers(),
							   componentPoint.x, componentPoint.y, e.getClickCount(), e.isPopupTrigger());
	        		menu.dispatchEvent(me);	        		    
        		}else if(componentPoint.y > menu.getHeight() && componentPoint.y <= (menu.getHeight()+display.getHeight()) && componentPoint.x <= display.getWidth()){
        			//Display
        			if(menu.isPopupMenuVisible()){
        				//Do something
        			}
        		}else if(componentPoint.y > menu.getHeight()+display.getHeight() && componentPoint.y <= menu.getHeight()+display.getHeight()+hide.getHeight() && componentPoint.x <= hide.getWidth()){		        			
        			//Hide
        			if(menu.isPopupMenuVisible()){
        				//Do something			        		
        			}
	...
	public static void main(String[] args){
		final TeleClientMenu tc = new TeleClientMenu();
