...
def drawBaseGraph(w_min, w_max, pstart, pend, oList, dynaOList):
    ...
    scene = display(title='State Time Graph', x=0, y=0, width=800, height=800, \
                    center=(xc,yc,zc), autoscale = True, userzoom = True, userspin = True)  
    for o in oList:
    ...
    points(pos=(pstart.x, pstart.y, pstart.z), size=10, color=color.green)
    points(pos=(pend.x, pend.y, pend.z), size=10, color=color.red)    
    #end for loop
#end drawBaseGraph
    
#start updateDrawAndScene
def updateDraw(dynaOList, curr, count):  
    points(pos=(curr.x, curr.y, curr.z), size=3, color=color.green)    
    for o in dynaOList:            
