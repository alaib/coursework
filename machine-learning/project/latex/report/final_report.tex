...
binding and non-binding peptides. The following subsections describe the dataset, features, optimization setup and the algorithm of my method.
\subsection{Dataset}
...
\begin{align}
\label{eqn}
\mathop{\textrm{minimize}}_{\ww^{1}, \ww^{2}} & \sum_i \mylog{1 + \myexp{-y^{1}_i(\xx^{1}_i\ww^{1})}} + \lambda_{1}\nrmo{\ww^{1}} \notag \\
                                              & + \sum_i \mylog{1 + \myexp{-y^{2}_i(\xx^{2}_i\ww^{2})}} + \lambda_{2}\nrmo{\ww^{2}} \\
                                              & + \mu\nrmo{\DD\ww} \notag.
\end{align}
...
Equation \eqref{eqn} can be further simplified by packing variables $\yy^1$ and $\yy^2$ into $\yy$, $\xx^1$ and $\xx^2$ into $\xx$ and suitably modifying $\DD$ as below:-
