...
mismatch_val2 = sum_of_squared_intensity_differences( baseimg, ...
...
[tempimg, xdata, ydata] = imtransform(movimg, tform, ...
                              'XData', [1 size(movimg,2)], 'YData', [1 size(movimg,1)]);
mismatch_val1 = sum_of_squared_intensity_differences( baseimg, ...
                                  tempimg, startx+1, startx+256, starty+1, starty+256);                                                         
...
[tempimg, xdata, ydata] = imtransform(movimg, tform, ...
                              'XData', [1 size(movimg,2)], 'YData', [1 size(movimg,1)]);
mismatch_val3 = sum_of_squared_intensity_differences( baseimg, ...
                                  tempimg, startx+1, startx+256, starty+1, starty+256);                              
skipFlag = false;                              
...
while true    
    ...
    [tempimg, xdata, ydata] = imtransform(movimg, tform, ...
                                  'XData', [1 size(movimg,2)], 'YData', [1 size(movimg,1)]);
    next_val = sum_of_squared_intensity_differences( baseimg, ...
                                      tempimg, startx+1, startx+256, starty+1, starty+256);
    if next_val > curr_val || deltax < -128  || deltax > 128
        %fprintf('deltax = %d, val = %e\n',deltax,next_val);
        deltax = deltax - step;
        count = count - 1;
        break
    end
